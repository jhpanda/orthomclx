...
def run_pipeline(config: RunConfig) -> None:
    ...
    parse_blast_to_compiled(config.blast_file, config.fasta_dir, compiled_dir)
    print(f"[orthomclx] compiled similarities written to {compiled_dir}")
    run_indexed_pairs(
        compiled_dir,
        config.out_dir,
