...
static RefVec load_or_build_refs(const char *binary_path, size_t record_count, const char *file_name, int (*cmp)(const void *, const void *)) {
  ...
  if (load_ref_file(path, record_count, &refs)) {
    fprintf(stderr, "[indexed_orthologs] loaded prebuilt refs: %s\n", file_name);
    return refs;
  }
  fprintf(stderr, "[indexed_orthologs] building refs in-process: %s\n", file_name);
...
static EdgeVec build_ortholog_edges(GroupVec *groups, RefVec *by_query_taxon, RefVec *by_query_subject, StringVec *proteins, float percent_match_cutoff, float cutoff_mant, int32_t cutoff_exp, uint32_t shard_index, uint32_t shard_count, bool raw_only) {
  EdgeVec edges = {0};
#ifdef _OPENMP
...
int main(int argc, char **argv) {
  ...
  RefVec by_query_taxon = load_or_build_refs(binary_path, g_records.len, "refs.query_taxon_best.bin", cmp_ref_query_taxon_best);
  ...
  EdgeVec orthologs = build_ortholog_edges(&groups, &by_query_taxon, &by_query_subject, &proteins, percent_match_cutoff, cutoff_mant, cutoff_exp, shard_index, shard_count, raw_only);
