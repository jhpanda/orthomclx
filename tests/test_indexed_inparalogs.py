...
class IndexedInparalogsTest(unittest.TestCase):
    def test_indexed_inparalog_stage_emits_expected_output(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            summary = (inpara_dir / "inparalogs.indexed.summary.tsv").read_text()
            output = (inpara_dir / "inparalogs.indexed.txt").read_text()
            self.assertIn("records\t12\n", summary)
            self.assertIn("inparalog_edges\t2\n", summary)
            self.assertEqual(
                output,
                "a1\ta2\t1.000\n"
                "b1\tb2\t1.000\n",
